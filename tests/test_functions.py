...
class TestFetchPageCaching:
    ...
    def test_cache_miss_fetches_and_then_stores_the_page(self, monkeypatch):
        monkeypatch.setattr(requests, "get", lambda url, timeout: FakePage())
        cache = FakeCache()
    ...
    def test_http_error_pages_are_not_cached(self, monkeypatch):
        ...
        monkeypatch.setattr(requests, "get", lambda url, timeout: ErrorPage())
        cache = FakeCache()
    ...
    def test_slashes_in_a_word_are_percent_encoded(self, monkeypatch):
        ...
        urls = []
        monkeypatch.setattr(
            requests, "get", lambda url, timeout: urls.append(url) or FakePage()
        )
...
class TestDownloadAudio:
    ...
    def test_uses_a_timeout_and_streams(self, monkeypatch, tmp_path):
        ...
        assert seen == {"timeout": func.REQUEST_TIMEOUT, "stream": True}
